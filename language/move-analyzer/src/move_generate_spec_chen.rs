...
impl FunSpecGenerator {
    //Possible parameter overflow and other anomalies in operations such as addition, subtraction, shift, etc
    //This function collects all the addition, subtraction, and other operations in e
        ...
        fn collect_spec_exp_(ret: &mut Vec<SpecExpItem>, e: &Exp) {
            match &e.value {
                Exp_::Call(n, _, tys, es) => {
                    let first_ty = tys.as_ref().and_then(|x| x.get(0));
                    ...
                    match &n.value {
                        NameAccessChain_::One(name) => match name.value.as_str() {
                            "borrow_global_mut" if first_ty.is_some() && first_e.is_some() => {
                                let ty = first_ty.unwrap().clone();
                                ret.push(SpecExpItem::BorrowGlobalMut {
                                    ty,
                                    addr: first_e.unwrap().clone(),
                                });
                            }
                ...
                Exp_::BinopExp(l, op, r) => {
                    ...
                    if let Some(reason) = BinOPReason::cause_exception(op.value) {
