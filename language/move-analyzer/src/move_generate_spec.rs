...
use crate::{item::MacroCall, project::Project, types::ResolvedType};
use move_compiler::{
    parser::ast::*,
    shared::{ast_debug::AstDebug, Identifier},
};
    ...
    fn try_emit_exp(
        ...
        match &e.value {
            Exp_::Call(_call, is_macro, should_be_none, es) => {
                ...
                {
                    if let std::result::Result::Ok(e) =
                        FunSpecGenerator::inverse_expression(es.value.get(0).unwrap())
                    {
                        if !FunSpecGenerator::emit_local_and_imports(
                            shadow,
                            statements,
                            &e,
                            imports,
...
impl FunSpecGenerator {
    fn expr_has_spec_unsupprted(e: &Exp) -> bool {
        fn exprs_has_spec_unsupprted(es: &Vec<Exp>) -> bool {
            es.iter().any(FunSpecGenerator::expr_has_spec_unsupprted)
        }
        match &e.value {
            Exp_::Value(_) => false,
            Exp_::Move(_) => false,
...
impl FunSpecGenerator {
    ...
    ) -> bool // emit ok ???
    {
        ...
        for (name, is_module) in {
            let mut x: Vec<_> = names.iter().map(|x| (*x, false)).collect();
            x.extend(
                modules
                    .iter()
                    .map(|x| (*x, true))
            ...
            if let Some(x) = shadow.query(name, is_module) {
                match x {
                    ...
                    ShadowItem::Local(index) => {
                        if !local_emited.contains(&index.index) {
...
impl GroupShadowItemUse {
    ...
    fn insert(&mut self, x: ShadowItemUse) {
        let k = match &x {
            ShadowItemUse::Module(x) => (x.lead, x.module),
            ShadowItemUse::Item(x) => (x.lead, x.module),
        };
    ...
    match u {
        Use::Module(addr_module, alias) => {
            ...
            let item = ShadowItem::Use(ShadowItemUse::Module(ShadowItemUseModule {
                lead: addr_module.value.address.value,
...
impl ShadowItems {
    ...
    fn query(&self, name: Symbol, module_name: bool) -> Option<&ShadowItem> {
        if module_name {
            ...
            self.items.get(&name).and_then(|x| x.last())
        }
    }
}
