...
impl MultiProject {
    ...
    pub fn get_project(&self, x: &Path) -> Option<&Project> {
        let (manifest, _) = super::utils_ori::discover_manifest_and_kind(x)?;
    ...
    fn get_projects_mut(&mut self, x: &Path) -> Vec<&mut Project> {
        let (manifest, _) = match super::utils_ori::discover_manifest_and_kind(x) {
            Some(x) => x,
            None => return vec![],
        };
        let mut ret = Vec::new();
    ...
    pub fn update_defs(&mut self, file_path: PathBuf, defs: Vec<Definition>) {
        let (manifest, layout) = match super::utils_ori::discover_manifest_and_kind(file_path.as_path())
        {
            Some(x) => x,
            None => {
