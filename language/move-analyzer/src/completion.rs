...
fn all_intrinsic() -> Vec<CompletionItem> {
    ...
    all.extend(aptos_framework_completion().into_iter());
    all
}

fn aptos_framework_completion() -> Vec<CompletionItem> {
    let mut ret = Vec::new();
    let x = String::from(
        r#"
fun init(ctx: &mut aptos::tx_context::TxContext) {
                        ...
                        move_compiler::parser::ast::NameAccessChain_::Three(
                                ...
                                || self.match_loc(&addr_and_module.loc, services)
                            {
                                let items = project_context.collect_modules_items(
                                    &addr,
                                    module.value,
                                    |x| matches!(x, Item::Struct(_) | Item::StructNameRef(_)),
                                );
                                push_items(self, &items);
                            }
                        }
                        ...
                        match &chain.value {
                            ...
                            move_compiler::parser::ast::NameAccessChain_::Three(
                                ...
                                if self.match_loc(&x.loc, services) {
                                    ...
                                    let items = project_context
                                        .collect_use_module_items(&x, |x| -> bool {
                                            matches!(x, Item::Fun(_))
                                        });
                                    if !items.is_empty() {
                                        push_items(self, &items);
                                    } else {
                                    ...
                                    let items = project_context.collect_modules_items(
                                        &addr,
                                        y.value,
                                        |x| matches!(x, Item::Fun(_) | Item::SpecSchema(_, _)),
                                    );
                                    push_items(self, &items);
                                }
                            }
